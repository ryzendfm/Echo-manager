...
export default function ClientOverviewCards() {
    ...
    useEffect(() => {
        const fetchStats = async () => {
            try {
                const res = await dashboardApi.getClientStats();
    ...
    const handleProjectClick = (project) => {
        closeModal();
        navigate("/client/projects", { state: { project } });
    };
    ...
    const cards = [
        {
            ...
            onClick: () => openModal("All Projects", Briefcase, stats?.totalProjectsList || [], "project", handleProjectClick, "No projects found."),
        },
        {
            ...
            onClick: () => openModal("Completed Projects", CheckCircle, stats?.completedProjectsList || [], "project", handleProjectClick, "No completed projects."),
        },
        {
            title: "Pending Invoices",
            value: fmtCurrency(stats?.pendingInvoicesAmount ?? 0),
            icon: FileText,
            description: `${stats?.pendingInvoicesCount ?? 0} invoice${(stats?.pendingInvoicesCount ?? 0) === 1 ? "" : "s"} due`,
